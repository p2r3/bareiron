...
uint8_t getChunkBiome (short x, short z) {
  ...
  dx = BIOME_RADIUS - mod_abs(x, BIOME_SIZE);
  dz = BIOME_RADIUS - mod_abs(z, BIOME_SIZE);
  /*
  ...
  biome_x = div_floor(x, BIOME_SIZE);
  biome_z = div_floor(z, BIOME_SIZE);
   ...
   */
  index = abs((biome_x & 3) + ((biome_z * 4) & 15));
...
uint8_t getTerrainAtFromCache (int x, int y, int z, int rx, int rz, ChunkAnchor anchor, ChunkFeature feature, uint8_t height) {
  ...
  if (y >= 64 && y >= height && feature.y != 255) switch (anchor.biome) {
    case W_plains: { /* Generate trees in the plains biome */
      ...
      dx = x > feature.x ? x - feature.x : feature.x - x;
      dz = z > feature.z ? z - feature.z : feature.z - z;
...
uint8_t getTerrainAt (int x, int y, int z, ChunkAnchor anchor) {
  ...
  feature = getFeatureFromAnchor(anchor);
  height = getHeightAtFromHash(rx, rz, anchor.x, anchor.z, anchor.hash, anchor.biome);
...
uint8_t getBlockAt (int x, int y, int z) {
  ...
  uint8_t block_change;
  ChunkAnchor anchor;
  if (y < 0) return B_bedrock;

  block_change = getBlockChange(x, y, z);
  if (block_change != 0xFF) return block_change;

  anchor.x = div_floor(x, CHUNK_SIZE);
  anchor.z = div_floor(z, CHUNK_SIZE);
...
uint8_t buildChunkSection (int cx, int cy, int cz) {
  ...
  for (i = cz; i < cz + 16 + CHUNK_SIZE; i += CHUNK_SIZE) {
    for (j = cx; j < cx + 16 + CHUNK_SIZE; j += CHUNK_SIZE) {
  ...
  for (i = 0; i < 16; i ++) {
    for (j = 0; j < 16; j ++) {
      ChunkAnchor *anchor_ptr;
      anchor_index = (j / CHUNK_SIZE) + (i / CHUNK_SIZE) * (16 / CHUNK_SIZE + 1);
      anchor_ptr = chunk_anchors + anchor_index;
