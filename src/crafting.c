...
void getSmeltingOutput (PlayerData *player) {
  ...
  uint16_t *material, *fuel, *output_item;
  uint8_t *output_count, fuel_value, exchange;
  ...
  material = &player->craft_items[0];
  fuel = &player->craft_items[1];

  /* Don't process if we're missing material or fuel */
  if (*material == 0 || *fuel == 0) return;

  /* Furnace output is 3rd crafting table slot */
  output_count = &player->craft_count[2];
  output_item = &player->craft_items[2];
  ...
  exchange = *material_count > fuel_value ? fuel_value : *material_count;
