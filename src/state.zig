...
pub const ServerContext = extern struct {
    ...
    // client_states mapping (fd, state)
    // matches C's ClientState
    // fd of -1 indicates unused slot
    client_states: [c.MAX_PLAYERS]extern struct {
        fd: c_int,
        state: c_int,
    },
...
pub const ServerState = struct {
    ...
    pub fn init() ServerState {
        ...
        // mark client state slots as unused
        var k: usize = 0;
        while (k < s.context.client_states.len) : (k += 1) {
            s.context.client_states[k].fd = -1;
        }
