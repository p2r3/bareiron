...
void handlePacket (int client_fd, int length, int packet_id, int state) {
  ...
  switch (packet_id) {
    ...
    case 0x20:
      if (state == STATE_PLAY) {
        ...
        double x, y, z;
        float yaw, pitch;
        uint8_t on_ground, block_feet, swimming, should_broadcast;
        short cx, cy, cz, _x, _y, _z, dx, dz;
        int found, count;
        uint32_t r;
        ...
        block_feet = getBlockAt(player->x, player->y, player->z);
        ...
        dx = _x - (player->x < 0 ? player->x - 16 : player->x) / 16;
        dz = _z - (player->z < 0 ? player->z - 16 : player->z) / 16;
        ...
        while (dz != 0) {
          ...
          total_ms = (double)(end - start) / CLOCKS_PER_SEC * 1000;
  ...
  processed_length = total_bytes_received - bytes_received_start;
...
int main () {
  int i, clients[MAX_PLAYERS], client_index = 0, server_fd, opt = 1, client_fd, length, packet_id, state;
  struct sockaddr_in server_addr, client_addr;
  socklen_t addr_len = sizeof(client_addr);
  int64_t last_tick_time, time_since_last_tick;
  ...
  while (true) {
    ...
    time_since_last_tick = get_program_time() - last_tick_time;
    ...
    client_fd = clients[client_index];
