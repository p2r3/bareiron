...
int cs_loginStart (int client_fd, uint8_t *uuid, char *name) {
  ...
  printf("  Player UUID: ");
  for (i = 0; i < 16; i ++) printf("%x", uuid[i]);
  printf("\n\n");
...
int sc_updateTime (int client_fd, uint64_t ticks) {
  ...
  world_age = get_program_time() / 50000;
...
int sc_chunkDataAndUpdateLight (int client_fd, int _x, int _z) {
  ...
  for (i = 0; i < 2048; i ++) chunk_section[i] = 0xFF;
  for (i = 2048; i < 4096; i ++) chunk_section[i] = 0;
  for (i = 0; i < 8; i ++) {
...
int cs_closeContainer (int client_fd) {
  ...
  for (i = 0; i < 9; i ++) {
    if (window_id != 2) {
      ...
      client_slot = serverSlotToClientSlot(window_id, 41 + i);
...
int cs_chat (int client_fd) {
  ...
  PlayerData *player;
  uint8_t has_signature, name_len;
  size_t message_len;
  int i;
  ...
  message_len = strlen((char *)recv_buffer);
  ...
  if (!strncmp((char *)recv_buffer, "!msg", 4)) {
    ...
    int text_offset = 0;
    int name_len, text_len, target_len;
    PlayerData *target;
    ...
    target = getPlayerByName(target_offset, target_end_offset, recv_buffer);
    if (target == NULL) {
    ...
    name_len = strlen(player->name);
    text_len = message_len - text_offset;
    ...
    target_len = target_end_offset - target_offset;
...
cleanup:
  ...
  has_signature = readByte(client_fd);
