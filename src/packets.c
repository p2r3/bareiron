...
int sc_statusResponse (ServerContext *ctx, int client_fd) {
  ...
  // Build a minimal, valid status JSON with version, players, and description
  // Note: MOTD is inserted as-is; avoid quotes in MOTD to keep JSON valid.
  char json[512];
  int len = snprintf(
    json, sizeof(json),
    ...
    MAX_PLAYERS,
    ctx->client_count,
    (int)ctx->motd_len, ctx->motd
  );
  if (len < 0) return 1;
  if (len > (int)sizeof(json)) len = (int)sizeof(json);

  writeVarInt(client_fd, 1 + len + sizeVarInt(len));
  writeByte(client_fd, 0x00);
  writeVarInt(client_fd, len);
  send_all(client_fd, json, len);
...
int cs_handshake (ServerContext *ctx, int client_fd) {
  ...
  printf("  Protocol version: %d\n", (int)readVarInt(ctx, client_fd));
  readString(ctx, client_fd);
  if (ctx->recv_count == -1) return 1;
  printf("  Server address: %s\n", ctx->recv_buffer);
  printf("  Server port: %u\n", readUint16(ctx, client_fd));
  int intent = readVarInt(ctx, client_fd);
  if (intent == VARNUM_ERROR) return 1;
  printf("  Intent: %d\n\n", intent);
  setClientState(ctx, client_fd, intent);
...
int cs_loginStart (ServerContext *ctx, int client_fd, uint8_t *uuid, char *name) {
  ...
  readString(ctx, client_fd);
  if (ctx->recv_count == -1) return 1;
  strncpy(name, (char *)ctx->recv_buffer, 16 - 1);
  name[16 - 1] = '\0';
  printf("  Player name: %s\n", name);
  ctx->recv_count = recv_all(client_fd, ctx->recv_buffer, 16, false);
  if (ctx->recv_count == -1) return 1;
  memcpy(uuid, ctx->recv_buffer, 16);
  printf("  Player UUID: ");
...
int cs_clientInformation (ServerContext *ctx, int client_fd) {
  ...
  if (ctx->recv_count == -1) return 1;
  printf("  Locale: %s\n", ctx->recv_buffer);
  tmp = readByte(ctx, client_fd);
  if (ctx->recv_count == -1) return 1;
  printf("  View distance: %d\n", tmp);
  tmp = readVarInt(ctx, client_fd);
  if (ctx->recv_count == -1) return 1;
  printf("  Chat mode: %d\n", tmp);
  tmp = readByte(ctx, client_fd);
  if (ctx->recv_count == -1) return 1;
...
int cs_pluginMessage (ServerContext *ctx, int client_fd) {
  ...
  if (ctx->recv_count == -1) return 1;
  printf("  Channel: \"%s\"\n", ctx->recv_buffer);
  if (strcmp((char *)ctx->recv_buffer, "minecraft:brand") == 0) {
    readString(ctx, client_fd);
    if (ctx->recv_count == -1) return 1;
    printf("  Brand: \"%s\"\n", ctx->recv_buffer);
  }
...
int sc_chunkDataAndUpdateLight (ServerContext *ctx, int client_fd, int _x, int _z) {
  ...
  for (int i = 0; i < ctx->block_changes_count; i ++) {
    #ifdef ALLOW_CHESTS
      if (ctx->block_changes[i].block != B_torch && ctx->block_changes[i].block != B_chest) continue;
    #else
      if (ctx->block_changes[i].block != B_torch) continue;
    #endif
    if (ctx->block_changes[i].x < x || ctx->block_changes[i].x >= x + 16) continue;
    if (ctx->block_changes[i].z < z || ctx->block_changes[i].z >= z + 16) continue;
...
int sc_blockUpdate (int client_fd, int64_t x, int64_t y, int64_t z, uint8_t block) {
  ...
  writeUint64(client_fd, ((uint64_t)(x & 0x3FFFFFF) << 38) | ((uint64_t)(z & 0x3FFFFFF) << 12) | (uint64_t)(y & 0xFFF));
...
int cs_playerAction (ServerContext *ctx, int client_fd) {
  ...
  uint8_t action = readByte(ctx, client_fd);

  int64_t pos = readInt64(ctx, client_fd);
  ...
  readByte(ctx, client_fd); // ignore face

  int sequence = readVarInt(ctx, client_fd);
  sc_acknowledgeBlockChange(client_fd, sequence);

  PlayerData *player;
  if (getPlayerData(ctx, client_fd, &player)) return 1;

  handlePlayerAction(ctx, player, action, x, y, z);
...
int cs_useItem (ServerContext *ctx, int client_fd) {
  ...
  uint8_t hand = readByte(ctx, client_fd);
  int sequence = readVarInt(ctx, client_fd);

  // Ignore yaw/pitch
  recv_all(client_fd, ctx->recv_buffer, 8, false);

  PlayerData *player;
  if (getPlayerData(ctx, client_fd, &player)) return 1;

  handlePlayerUseItem(ctx, player, 0, 0, 0, 255);
...
int cs_useItemOn (ServerContext *ctx, int client_fd) {
  ...
  uint8_t face = readByte(ctx, client_fd);

  // ignore cursor position
  readUint32(ctx, client_fd);
  readUint32(ctx, client_fd);
  readUint32(ctx, client_fd);

  // ignore "inside block" and "world border hit"
  readByte(ctx, client_fd);
  readByte(ctx, client_fd);
  ...
  handlePlayerUseItem(ctx, player, x, y, z, face);
...
int cs_clickContainer (ServerContext *ctx, int client_fd) {
  int window_id = readVarInt(ctx, client_fd);

  readVarInt(ctx, client_fd); // ignore state id

  int16_t clicked_slot = readInt16(ctx, client_fd);
  uint8_t button = readByte(ctx, client_fd);
  uint8_t mode = readVarInt(ctx, client_fd);

  int changes_count = readVarInt(ctx, client_fd);
  ...
  slot = clientSlotToServerSlot(window_id, readUint16(ctx, client_fd));
  ...
  if (!readByte(ctx, client_fd)) { // no item?
      if (slot != 255 && apply_changes) {
        ...
        *p_count = 0;
        #ifdef ALLOW_CHESTS
        if (window_id == 2 && slot > 40) {
          broadcastChestUpdate(ctx, client_fd, storage_ptr, 0, 0, slot - 41);
        }
        #endif
      }
      continue;
  ...
  item = readVarInt(ctx, client_fd);
  count = (uint8_t)readVarInt(ctx, client_fd);
  ...
  recv_all(client_fd, ctx->recv_buffer, tmp, false);
    ...
    if (count > 0 && apply_changes && slot != 255) {
      ...
      *p_count = count;
      #ifdef ALLOW_CHESTS
      if (window_id == 2 && slot > 40) {
        broadcastChestUpdate(ctx, client_fd, storage_ptr, item, count, slot - 41);
      }
      #endif
    }
  ...
  if (readByte(ctx, client_fd)) {
    player->flagval_16 = readVarInt(ctx, client_fd);
    player->flagval_8 = readVarInt(ctx, client_fd);
    // ignore components
    tmp = readVarInt(ctx, client_fd);
    recv_all(client_fd, ctx->recv_buffer, tmp, false);
    tmp = readVarInt(ctx, client_fd);
...
int cs_setPlayerPositionAndRotation (ServerContext *ctx, int client_fd, double *x, double *y, double *z, float *yaw, float *pitch, uint8_t *on_ground) {
  ...
  *x = readDouble(ctx, client_fd);
  *y = readDouble(ctx, client_fd);
  *z = readDouble(ctx, client_fd);

  *yaw = readFloat(ctx, client_fd);
  *pitch = readFloat(ctx, client_fd);

  *on_ground = readByte(ctx, client_fd) & 0x01;
...
int cs_setPlayerMovementFlags (ServerContext *ctx, int client_fd, uint8_t *on_ground) {
  ...
  *on_ground = readByte(ctx, client_fd) & 0x01;
  ...
  if (!getPlayerData(ctx, client_fd, &player))
    broadcastPlayerMetadata(ctx, player);
...
int cs_swingArm (ServerContext *ctx, int client_fd) {
  ...
  uint8_t hand = readVarInt(ctx, client_fd);
...
int cs_setHeldItem (ServerContext *ctx, int client_fd) {
  ...
  {
  uint16_t slot = readUint16(ctx, client_fd);
    if (slot > 8) slot = 0;
...
int cs_closeContainer (ServerContext *ctx, int client_fd) {
  ...
  uint8_t window_id = readVarInt(ctx, client_fd);
...
int cs_clientStatus (ServerContext *ctx, int client_fd) {
  ...
  uint8_t id = readByte(ctx, client_fd);
...
int cs_chat (ServerContext *ctx, int client_fd) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i ++) {
    ...
    sc_systemChat(ctx->player_data[i].client_fd, (char *)ctx->recv_buffer, message_len + name_len + 3);
  }
...
int cs_interact (ServerContext *ctx, int client_fd) {
  int entity_id = readVarInt(ctx, client_fd);
  uint8_t type = readByte(ctx, client_fd);

  if (type == 2) {
    // Ignore target coordinates
    recv_all(client_fd, ctx->recv_buffer, 12, false);
  }
  if (type != 1) {
    // Ignore hand
...
int cs_playerInput (ServerContext *ctx, int client_fd) {
  ...
  uint8_t flags = readByte(ctx, client_fd);
...
int cs_playerCommand (ServerContext *ctx, int client_fd) {
  ...
  readVarInt(ctx, client_fd); // Ignore entity ID
  uint8_t action = readByte(ctx, client_fd);
  readVarInt(ctx, client_fd); // Ignore "Jump Boost" value
...
int cs_playerLoaded (ServerContext *ctx, int client_fd) {
  ...
  // Only broadcast join if still marked as loading
